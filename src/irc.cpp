...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #CapstoneTEST2\r");
            Send(hSocket, "WHO #CapstoneTEST2\r");
        } else {
            // randomly join #Capstone00-#Capstone05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Capstone%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Capstone%02d\r", channel_number).c_str());
        }
