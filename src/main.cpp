...
unsigned int nStakeMinAge = 33 * 24 * nOneHour; // 33 days as zero time weight
unsigned int nStakeMaxAge = 99 * 24 * nOneHour; // 99 days as full weight
unsigned int nStakeTargetSpacing = 333; // blocktime0-minute stakes spacing
int64_t nTargetSpacing = 333;  // Same as the above
...
const string strMessageMagic = "Capstone Signed Message:\n";
...
{
    int64_t nSubsidy = 33 * COIN;
    
	// Force block reward to zero when right shift is undefined.
    int halvings = nHeight / 3333333;
    if (halvings >= 64)
        return nFees;
...
{
    int64_t nRewardCoinYear = 33 * CENT;
...
static const int64_t nTargetTimespan = (333 * 15);
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("Capstone-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Capstone
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Capstone: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Capstone", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Capstone Genesis";
        CTransaction txNew;
        txNew.nTime = 1462214133;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1462214140;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 362229; // !fTestNet ? 1575379 : 46534;
		

        block.print();

        //// debug print
        assert(block.hashMerkleRoot == uint256("0x0cdbf80ad221249bde78c364bbe779dd32b3fccffd859bb916de6c140edc9c79"));
...
unsigned char pchMessageStart[4] = { 0xaa, 0xf5, 0xb2, 0xe5 };
